import { GetCommand, PutCommand } from "@aws-sdk/lib-dynamodb";
import { mockClient } from "aws-sdk-client-mock";
import { beforeEach, describe, expect, test, vi } from "vitest";

import awsClients from "../../shared/clients/aws";
import { logger } from "../../shared/logger";
import {
  ChestXRayNotTakenReason,
  HistoryOfConditionsUnder11,
  MenstrualPeriods,
  PregnancyStatus,
  TbSymptomsOptions,
  YesOrNo,
} from "../types/enums";
import {
  MedicalScreeningDbOps,
  NewMedicalScreeningChestXray,
  NewMedicalScreeningNoChestXray,
} from "./medical-screening";

describe("Tests for Medical Screening Information Model", () => {
  const ddbMock = mockClient(awsClients.dynamoDBDocClient);

  beforeEach(() => {
    ddbMock.reset();
  });

  const newMedicalScreening: NewMedicalScreeningChestXray = {
    dateOfMedicalScreening: "2025-05-05",
    age: 23,
    applicationId: "test-application-id",
    symptomsOfTb: YesOrNo.Yes,
    symptoms: [TbSymptomsOptions.Fever, TbSymptomsOptions.Haemoptysis],
    historyOfConditionsUnder11: [HistoryOfConditionsUnder11.Cyanosis],
    historyOfConditionsUnder11Details: "empty result",
    historyOfPreviousTb: YesOrNo.Yes,
    contactWithPersonWithTb: YesOrNo.No,
    pregnant: PregnancyStatus.DontKnow,
    haveMenstralPeriod: MenstrualPeriods.NA,
    physicalExaminationNotes: "No notes",
    createdBy: "test-medical-screening-creator",
    isXrayRequired: YesOrNo.Yes,
  };

  const newMedicalScreeningNoXray: NewMedicalScreeningNoChestXray = {
    dateOfMedicalScreening: "2025-05-05",
    age: 23,
    applicationId: "test-application-id",
    symptomsOfTb: YesOrNo.Yes,
    symptoms: [TbSymptomsOptions.Fever, TbSymptomsOptions.Haemoptysis],
    historyOfConditionsUnder11: [HistoryOfConditionsUnder11.Cyanosis],
    historyOfConditionsUnder11Details: "empty result",
    historyOfPreviousTb: YesOrNo.Yes,
    contactWithPersonWithTb: YesOrNo.No,
    pregnant: PregnancyStatus.DontKnow,
    haveMenstralPeriod: MenstrualPeriods.NA,
    physicalExaminationNotes: "No notes",
    createdBy: "test-medical-screening-creator",
    isXrayRequired: YesOrNo.No,
    reasonXrayNotRequired: ChestXRayNotTakenReason.Other,
  };
  test("Creating new medical sreening with Xray required", async () => {
    // Arrange
    ddbMock.on(PutCommand);
    vi.useFakeTimers();
    const expectedDateTime = "2025-03-04";
    vi.setSystemTime(expectedDateTime);

    // Act
    const medicalScreening =
      await MedicalScreeningDbOps.createMedicalScreening(newMedicalScreening);

    // Assert
    expect(medicalScreening).toMatchObject({
      ...newMedicalScreening,
      dateOfMedicalScreening: new Date(newMedicalScreening.dateOfMedicalScreening),
      dateCreated: new Date(expectedDateTime),
    });

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    expect(ddbMock.commandCalls(PutCommand)[0].firstArg.input).toMatchObject({
      TableName: "test-application-details",
      Item: {
        ...newMedicalScreening,
        dateOfMedicalScreening: new Date(newMedicalScreening.dateOfMedicalScreening).toISOString(),
        pk: "APPLICATION#test-application-id",
        sk: "APPLICATION#MEDICAL#SCREENING",
      },
    });
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    expect(ddbMock.commandCalls(PutCommand)[0].firstArg.input).toMatchObject({
      ConditionExpression: "attribute_not_exists(pk) AND attribute_not_exists(sk)",
    });
  });

  test("Creating new medical sreening with Xray Not required", async () => {
    // Arrange
    ddbMock.on(PutCommand);
    vi.useFakeTimers();
    const expectedDateTime = "2025-03-04";
    vi.setSystemTime(expectedDateTime);
    // Act
    const medicalScreening =
      await MedicalScreeningDbOps.createMedicalScreening(newMedicalScreeningNoXray);

    // Assert
    expect(medicalScreening).toMatchObject({
      ...newMedicalScreeningNoXray,
      dateOfMedicalScreening: new Date(newMedicalScreeningNoXray.dateOfMedicalScreening),
      dateCreated: new Date(expectedDateTime),
    });

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    expect(ddbMock.commandCalls(PutCommand)[0].firstArg.input).toMatchObject({
      TableName: "test-application-details",
      Item: {
        ...newMedicalScreeningNoXray,
        dateOfMedicalScreening: new Date(
          newMedicalScreeningNoXray.dateOfMedicalScreening,
        ).toISOString(),
        pk: "APPLICATION#test-application-id",
        sk: "APPLICATION#MEDICAL#SCREENING",
      },
    });
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    expect(ddbMock.commandCalls(PutCommand)[0].firstArg.input).toMatchObject({
      ConditionExpression: "attribute_not_exists(pk) AND attribute_not_exists(sk)",
    });
  });

  test.each([
    {
      newMedicalScreening: newMedicalScreening,
      title: "Medical Screening with Chest Xray Required",
    },
    {
      newMedicalScreening: newMedicalScreeningNoXray,
      title: "Medical Screening with Chest Xray Not Required",
    },
  ])("Getting medical screening by application ID", async ({ newMedicalScreening }) => {
    const dateCreated = "2025-02-07";
    ddbMock.on(GetCommand).resolves({
      Item: {
        ...newMedicalScreening,
        dateCreated,
        pk: "APPLICATION#test-application-id",
        sk: "APPLICATION#MEDICAL#SCREENING",
      },
    });

    // Act
    const medicalScreening = await MedicalScreeningDbOps.getByApplicationId(
      newMedicalScreening.applicationId,
    );

    // Assert
    expect(medicalScreening).toMatchObject({
      ...medicalScreening,
      dateCreated: new Date("2025-02-07"),
    });
  });

  test("Handling error while getting medical screening by applicationId", async () => {
    const errorLoggerMock = vi.spyOn(logger, "error").mockImplementation(() => null);
    ddbMock.on(GetCommand).rejects(Error("DB error"));

    // Act / Assert
    await expect(
      MedicalScreeningDbOps.getByApplicationId(newMedicalScreening.applicationId),
    ).rejects.toThrow("DB error");
    expect(errorLoggerMock).toHaveBeenCalledWith(
      Error("DB error"),
      "Error retrieving medical screening details",
    );
  });
});
