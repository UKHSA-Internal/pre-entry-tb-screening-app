...
def test_load_clinics_data_integration_cp1252(tables, test_csv, tmp_path, monkeypatch):
  # Simulate S3 by using a local file and a mock client
  class S3Mock:
    def get_object(self, Bucket, Key):
      csv_bytes = test_csv.encode("cp1252")
...
def test_load_clinics_data_integration_no_encoding(tables, test_csv, tmp_path, monkeypatch):
  # Simulate S3 by using a local file and a mock client
  class S3Mock:
    def get_object(self, Bucket, Key):
      csv_bytes = test_csv.encode()
      return {"Body": io.BytesIO(csv_bytes), "ContentLength": len(csv_bytes)}
