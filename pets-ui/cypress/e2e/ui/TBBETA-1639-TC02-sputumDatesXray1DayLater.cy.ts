//PETS Date Validation Test: Valid Scenario - X-ray 1 Day After Medical
// Biz Rule 1: Medical screening on clinic visit date, X-ray 1 day later (acceptable)
...
describe("PETS Date Validation: Valid Scenario - X-ray 1 Day After Medical", () => {
  ...
  before(() => {
    ...
    // Generate dynamic dates for adult applicant (26 years old)
    adultAge = 26;
    ...
    // X-RAY 1 DAY AFTER MEDICAL SCENARIO: Medical screening 5 days ago, X-ray 4 days ago
    const screening = DateUtils.getDateInPast(0, 0, 5); // 5 days ago (Day 0)
    screeningDate = DateUtils.getDateComponents(screening);

    // X-ray 1 day after medical screening
    const xray = DateUtils.getDateInPast(0, 0, 4); // 4 days ago (Day 1)
    xrayDate = DateUtils.getDateComponents(xray);
    xrayDateFormatted = DateUtils.formatDateGOVUK(xray);

    // Sputum collection dates: consecutive days after X-ray (Days 2, 3, 4)
    const sample1 = DateUtils.getDateInPast(0, 0, 3); // 3 days ago (Day 2)
    const sample2 = DateUtils.getDateInPast(0, 0, 2); // 2 days ago (Day 3)
    const sample3 = DateUtils.getDateInPast(0, 0, 1); // 1 day ago (Day 4)
    ...
    cy.log("=== X-RAY 1 DAY AFTER MEDICAL SCENARIO ===");
    cy.log(`Medical Screening: Day 0 (5 days ago)`);
    cy.log(`X-ray: Day 1 (4 days ago - 1 day after Medical)`);
    cy.log(`Sputum Sample 1: Day 2 (3 days ago)`);
    cy.log(`Sputum Sample 2: Day 3 (2 days ago)`);
    cy.log(`Sputum Sample 3: Day 4 (1 day ago)`);
    cy.log(`Total Duration: 4 days (within 7-day window)`);
  ...
  it("should successfully complete TB screening with X-ray 1 day after medical, Sputum consecutive days 2,3,4", () => {
    ...
    checkChestXrayImagesPage.getUploadedImages().then((images) => {
      ...
      chestXrayFindingsPage.selectMinorFindingByIndex(4); // Select "2.3 Calcified nodule(s)"
      ...
      cy.log("✓ X-RAY 1 DAY AFTER MEDICAL SCENARIO TEST PASSED - All dates valid and within rules");
    });
  });
});
