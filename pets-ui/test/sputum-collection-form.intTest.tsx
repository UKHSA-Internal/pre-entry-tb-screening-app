...
describe("SputumCollectionForm", () => {
  ...
  test("renders the form with initial (empty) values from Redux store", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Sputum collection details", level: 1 }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByText("Save and enter results", { selector: "button" })).toBeInTheDocument();
  });
  ...
  test("updates store and navigates to '/check-sputum-collection-details-results' on 'Save progress' with valid data for all samples", async () => {
  ...
  test("shows correct sample headings and structure", () => {
    ...
    expect(screen.getAllByRole("heading", { name: "Date collected", level: 3 })).toHaveLength(3);
