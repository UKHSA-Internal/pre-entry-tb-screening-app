...
describe("MedicalScreeningReview", () => {
  ...
  test("state is displayed correctly & user is navigated to confirmation page when medical details are posted successfully", async () => {
    ...
    expect(screen.getAllByRole("definition")[4]).toHaveTextContent("99");
    expect(screen.getAllByRole("term")[5]).toHaveTextContent(
      "Does the applicant have TB symptoms?",
    );
    expect(screen.getAllByRole("definition")[6]).toHaveTextContent("Yes");
    expect(screen.getAllByRole("term")[6]).toHaveTextContent("TB symptoms");
    expect(screen.getAllByRole("definition")[8]).toHaveTextContent("Cough, Night sweats");
    ...
    expect(screen.getAllByRole("definition")[21]).toHaveTextContent("Don't know");
