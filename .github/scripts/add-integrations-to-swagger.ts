module.exports = (serviceName: string) => {
    ...
    let inputFilePath: string = `core-services/${serviceName}/swagger/swagger.js`
    const fs = require('fs')
    let swaggerAsArray: string[] = fs.readFileSync(inputFilePath, 'utf-8').split('\n')
    swaggerAsArray = swaggerAsArray.slice(2,-1)
    let swaggerAsString: string = '{' + swaggerAsArray.join('') + '}'
    let swaggerAsJson: JSON = JSON.parse(swaggerAsString)

    // Define integration block
    let integrationBlock: Object = {
        'uri': '',
    ...
    let swaggerSpec: string = JSON.stringify(swaggerAsJson)
