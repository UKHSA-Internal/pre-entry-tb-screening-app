module.exports = (serviceName) => {
  ...
  let securityDefinitions = {
    authorizer: {
      ...
      "x-amazon-apigateway-authorizer": {
        type: "token",
        authorizerUri:
          "arn:aws:apigateway:eu-west-2:lambda:path/2015-03-31/functions/arn:aws:lambda:eu-west-2:AWS_ACCOUNT_ID:function:api-gateway-authoriser/invocations",
  ...
  for (let path in swaggerAsJson["paths"]) {
    for (let httpMethod in swaggerAsJson["paths"][path]) {
      ...
      swaggerAsJson["paths"][path][httpMethod]["x-amazon-apigateway-integration"] = {
        ...integrationBlock,
      }; // Spreading the object clones it, preventing original object from being referenced when fields are amended
      ...
      swaggerAsJson["paths"][path][httpMethod]["x-amazon-apigateway-integration"]["uri"] =
        uriSecretName;
      swaggerAsJson["paths"][path][httpMethod]["x-amazon-apigateway-integration"]["httpMethod"] =
  ...
  for (let uri of uriSecretNames) {
    swaggerSpec = swaggerSpec.replace(uri, eval("process.env." + uri));
  }
  fs.writeFile(`core-services/${serviceName}/swagger.json`, swaggerSpec, (err) => {
    if (err) throw err;
  });
};
