...
describe('Radio component', () => {
    it('renders correctly when all optional props are specified', () => {
        render(<Radio id='test-id' title='test-title' legend='test-legend' hint='test-hint' isInline={RadioIsInline.FALSE} answerOptions={['zzz Answer One', 'aaa Answer Two']} sortAnswersAlphabetically={false}/>)
    ...
    it('renders correctly when all optional props are omitted', () => {
        render(<Radio id='test-id' isInline={RadioIsInline.FALSE} answerOptions={['zzz Answer One', 'aaa Answer Two']} sortAnswersAlphabetically={false}/>)
        expect(screen.getAllByRole('radio')).toBeTruthy()
    ...
    it('is not inline when props.isInline is FALSE', () => {
        const {container} = render(<Radio id='test-id' isInline={RadioIsInline.FALSE} answerOptions={['zzz Answer One', 'aaa Answer Two']} sortAnswersAlphabetically={false}/>)
    ...
    it('is inline when props.isInline is TRUE', () => {
        const {container} = render(<Radio id='test-id' isInline={RadioIsInline.TRUE} answerOptions={['zzz Answer One', 'aaa Answer Two']} sortAnswersAlphabetically={false}/>)
        expect(container.getElementsByClassName('govuk-radios')).toHaveLength(1)
    ...
    it('orders answers alphabetically when props.sortAnswersAlphabetically is true', () => {
        const {container} = render(<Radio id='test-id' isInline={RadioIsInline.FALSE} answerOptions={['zzz Answer One', 'aaa Answer Two']} sortAnswersAlphabetically={true}/>)
        const labels = container.getElementsByClassName('govuk-label govuk-radios__label')
